...
namespace ApiFipe.Controllers
{
    ...
    public class PropostaController : ControllerBase
    {
                            ...
                            if (item.Coordenadores.Count <= 0)
                            {
                                ...
                                foreach (var coordenador in item.Coordenadores)
                                {
                                    ...
                                    if (new bProposta(db)._PodeEnviarEmail(item.IdProposta, coordenador, pessoaFisica.CdEmail))
                                    {
                                        ...
                                        //{
                                        //EGS 31.03.2021 Marcio pediu para tambem enviar email para a Ilana
                                        ...
                                        _GLog._GravaLog(AppSettings.constGlobalUserID, "Email de Proposta [" + proposta.IdProposta + "] para Responsavel (com Ilana) enviado para [" + "marcio.rodrigues@fipe.org.br" + "]");
                        ...
                        try
                         {
                            ...
                            proposta.IdUsuarioUltimaAlteracao = item.IdUsuarioAprovador;
            ...
            using (var db = new FIPEContratosContext())
            {
                ...
                strategy.Execute(() =>
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            int bNovoUsuario                  = (Int32)item.IdUsuarioUltimaAlteracao;   //EGS 06.03.2021 Sabadão
                            var proposta                      = new bProposta(db).BuscarPropostaId(item.IdProposta);
                            var usuariosGestorProposta        = new bUsuario(db).GetGestoresProposta();
                            var emailAprovacao                = new bEmail(db).GetEmailById(17);
                            var linkAprovar                   = item.Url + "/proposta/" + item.IdProposta;
                            proposta.IdSituacao               = 26;
                            proposta.IdUsuarioUltimaAlteracao = bNovoUsuario; // item.IdUsuarioUltimaAlteracao;
                            ...
                            newPropostaHist.IdProposta = proposta.IdProposta;
                            newPropostaHist.DtInicio   = DateTime.Now;
                            newPropostaHist.IdUsuario  = bNovoUsuario; // AppSettings.constGlobalUserID;
                            newPropostaHist.IdSituacao = proposta.IdSituacao; 
                            db.PropostaHistorico.Add(newPropostaHist);
                            db.SaveChanges();
                            db.Database.CommitTransaction();
                          ...
                          //_GLog._GravaLog(AppSettings.constGlobalUserID, "SolicitarAjustesDiretoria  Historico [" + proposta.IdProposta + "] criada com situacao [" + proposta.IdSituacao + "] Usu Info [" + (Int32)item.IdUsuarioUltimaAlteracao + "] e constGlobalUserID [" + AppSettings.constGlobalUserID + "]");
                            _GLog._GravaLog(bNovoUsuario                 , "SolicitarAjustesDiretoriaX Historico [" + proposta.IdProposta + "] criada com situacao [" + proposta.IdSituacao + "] Usu Info [" + (Int32)item.IdUsuarioUltimaAlteracao + "] e constGlobalUserID [" + AppSettings.constGlobalUserID + "]");
                    ...
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                          ...
                          //_GLog._GravaLog(AppSettings.constGlobalUserID, "SolicitarAjustesPropostaJuridico  Historico [" + proposta.IdProposta + "] criada com situacao [" + proposta.IdSituacao + "] Usu Info [" + (Int32)item.IdUsuarioUltimaAlteracao + "] e constGlobalUserID [" + AppSettings.constGlobalUserID + "]");
                            _GLog._GravaLog(bNovoUsuario                 , "SolicitarAjustesPropostaJuridicoX Historico [" + proposta.IdProposta + "] criada com situacao [" + proposta.IdSituacao + "] Usu Info [" + (Int32)item.IdUsuarioUltimaAlteracao + "] e constGlobalUserID [" + AppSettings.constGlobalUserID + "]");
                            db.SaveChanges();
                            db.Database.CommitTransaction();
        ...
        public InputUpdateProposta BuscaPropostaId(int id)
        {
            using (var db = new FIPEContratosContext())
            {
                ...
                try
                {
                    ...
                    //EGS 08.03.2021Estava dando erro
        ...
        public OutPutListaPropostas ListaPropostas(int idUsuario)
        {
            using (var db = new FIPEContratosContext())
            {
                try
                {
                    ...
                    //EGS 30.03.2021 Estava faltando essa parte no codigo do Alessandro :(
                    foreach (var itemProp in listaPropostas)
                    {
                        if (itemProp.IdTipoOportunidade == 7)
                        {
                            retornoPropostas.TamanhoPropostasAditivos += 1;
                ...
                strategy.Execute(() =>
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            _GLog._GravaLog(AppSettings.constGlobalUserID, "Update Proposta [" + item.IdProposta + "] Últ.Usua [" + item.IdUsuarioUltimaAlteracao + "] e constGlobalUserID [" + AppSettings.constGlobalUserID + "] e sit [" + item.IdSituacao + "]");
            ...
            using (var db = new FIPEContratosContext())
            {
                try
                {
                    ...
                    foreach (var pJuridica in pessoasJuridicas)
                    {
                        ...
                        if (pessoa == null)
                        {
                            var iID = pJuridica.IdPessoaJuridica;
                        } else { 
                ...
                strategy.Execute(() =>
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            contratoAditivo.NuAditivoCliente    = (short)nuAditivo;
                            ...
                            //----------------------- 30.04.2021 Enviar Email para Contratos ao finalizar aditivo --------------------------------
                            var itemURL = "https://sgpc.fipe.org.br";
                            if (!FIPEContratosContext.EnvironmentIsProduction) { itemURL = "https://sgpc.cit-homolog.com.br"; }

                            var emailAditivo      = new bEmail(db).GetEmailByTitulo("Proposta de Aditivo {0} encaminhada para Contratos");
                            var linkVisualizar    = itemURL + "/proposta/" + proposta.IdProposta;
                            var usuarContratos    = db.PerfilUsuario.Where(p => p.IdPerfil == 4 && p.EnviaEmail == true).ToList();     //EGS 30.04.2021 So envia email se esta marcado de Contratos
                            var assuntoAditivo    = String.Format(emailAditivo.DsTitulo, proposta.IdProposta);
                            new bProposta(db)._ValidaDataFinalHistorico(proposta.IdProposta);
                            foreach (var usuarioDiretor in usuarContratos)
                            {
                                var usuario = db.Usuario.Where     (w => w.IdUsuario      == usuarioDiretor.IdUsuario).FirstOrDefault();
                                var pessoa  = db.PessoaFisica.Where(w => w.IdPessoaFisica == usuario.IdPessoa        ).FirstOrDefault();
                                var corpo   = String.Format(emailAditivo.DsTexto, pessoa.NmPessoa, proposta.IdProposta, linkVisualizar);

                                new bEmail(db).EnviarEmail(pessoa.CdEmail, pessoa.NmPessoa, assuntoAditivo, corpo, null, string.Empty);
                                _GLog._GravaLog(AppSettings.constGlobalUserID, "Email de Proposta de Aditivo [" + proposta.IdProposta + "] para Contratos enviado para [" + pessoa.CdEmail + "]");
                            }
                ...
                strategy.Execute(() =>
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // Confirma operações
                            db.Database.CommitTransaction();
                            _GLog._GravaLog(AppSettings.constGlobalUserID, "UploadDocAditivo adicionado no Contrato [" + IdContrato + "] criador [" + NmCriador + "]");
                ...
                strategy.Execute(() =>
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // Confirma operações
                            db.Database.CommitTransaction();
                            _GLog._GravaLog(AppSettings.constGlobalUserID, "UpLoadDoc adicionado na Proposta [" + IdProposta + "] criador [" + NmCriador + "]");
                ...
                strategy.Execute(() =>
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            db.PropostaDocsPrincipais.Add(itemDoc);
                            db.SaveChanges();
                            db.Database.CommitTransaction();
                            _GLog._GravaLog(AppSettings.constGlobalUserID, "AddDocumentoPrincipal Prop [" + IdProposta + "] - Doc [" + itemDoc.NmDocumento + "] Gravada com sucesso");

                            return Ok();
                        }
                        catch (Exception ex)
                        {
                            new bEmail(db).EnviarEmailTratamentoErro(ex, "PropostaController-UpLoadDocPrincipal IDProp [" + IdProposta + "] Criador: " + NmCriador);
                            throw;
                        }
                    }
                });
        ...
        *  Edinaldo FIPE
        *  Maio/2020 
        *  Verifica se a proposta JA ESTA no status informado
        ==============================================================================================*/
        [ServiceFilter(typeof(AutenticacaoActionFilter))]
        [HttpGet]
        [Route("ValidarSePropostaEstanoStatus/{pIDProposta}/{pIDSituacao}")]
        public bool ValidarSePropostaEstanoStatus(int pIDProposta, int pIDSituacao)
        {
            bool bRetorno = true;
            using (var db = new FIPEContratosContext())
            {
                _GLog._GravaLog(AppSettings.constGlobalUserID, "Verificando se a Proposta [" + pIDProposta + "] já esta com a Situacao [" + pIDSituacao + "]");
                try
                {
                    var propostaStatus = db.Proposta.Where(w => w.IdProposta == pIDProposta).FirstOrDefault();
                    if (propostaStatus != null)
                    {
                        if (propostaStatus.IdSituacao == pIDSituacao)
                        {
                            _GLog._GravaLog(AppSettings.constGlobalUserID, "Proposta [" + pIDProposta + "] Já esta com a Situacao [" + pIDSituacao + "]. NÃO sera alterado a situacao");
                            bRetorno = false;
                        }
                    }
                }
                ...
                {
                    new bEmail(db).EnviarEmailTratamentoErro(ex, "PropostaController-ValidarSePropostaEstanoStatus Proposta [" + pIDProposta + "] na Situacao [" + pIDSituacao + "]");
                    throw;
                }
